...
export default function UserDashboard() {
  ...
  const handleStartAssessment = () => {
    if (definitions.length > 0 && user) {
      const instanceId = createInstance(definitions[0].id, user.id);
      navigate(`/assessment/${definitions[0].id}?instance=${instanceId}`);
    }
  };
  ...
  const handleContinueAssessment = (definitionId: string, instanceId: string) => {
    navigate(`/assessment/${definitionId}?instance=${instanceId}`);
  };
